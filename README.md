...
MP4動画ファイルから音声を抽出し、文字起こしに最適な音質に加工するPython製ツールです。

**2つの使用方法:**
- 🖥️ **CLI版** (`audio_processor.py`) - コマンドラインで実行
- 🌐 **WebUI版** (`audio_processor_gui.py`) - ブラウザで簡単操作！
...
- **使いやすいWebUI**: ブラウザで直感的に操作可能（プリセット機能付き）
...
ブラウザが自動的に開き、GUIで操作できます。

**WebUI版の機能:**
- ドラッグ&ドロップでファイルアップロード
- スライダーで直感的にパラメータ調整
- プリセット機能（標準/高品質/高速/積極的）
- リアルタイムで処理状況を確認
- 処理済み音声をその場で再生
- ワンクリックでダウンロード
- **FFmpeg設定機能** - 自動検出とカスタムパス設定

**プリセット:**
- 📝 **標準（バランス型）**: ノイズ除去と無音除去のバランスが良い設定
- ⭐ **高品質（ノイズ除去重視）**: 音質を最優先した設定
- ⚡ **高速（処理速度重視）**: ノイズ除去をスキップして高速処理
- ✂️ **積極的（無音削除重視）**: 無音を徹底的に削除

**FFmpeg設定:**
- 🔧 **自動検出**: 起動時にFFmpegの状態を自動確認
- 📍 **パス表示**: FFmpegのインストール場所とバージョンを表示
- ✅ **カスタムパス**: システムで見つからない場合は手動で設定可能
...
#### WebUI版の場合
1. WebUIの「🔧 FFmpeg設定」セクションを開く
2. 「🔄 FFmpegを再確認」ボタンをクリックして現在の状態を確認
3. FFmpegが見つからない場合：
   - **方法1**: FFmpegをインストールして環境変数PATHに追加（推奨）
   - **方法2**: カスタムFFmpegパスを手動で設定
     - FFmpegの実行ファイルのフルパスを入力
     - 例: `C:\ffmpeg\bin\ffmpeg.exe` (Windows)
     - 例: `/usr/local/bin/ffmpeg` (Mac/Linux)
     - 「✅ カスタムパスを設定」ボタンをクリック

#### CLI版の場合
- FFmpegをインストールしてください（上記の「FFmpegのインストール」を参照）
- インストール後、環境変数PATHが正しく設定されているか確認してください
- コマンドラインで `ffmpeg -version` を実行して確認
